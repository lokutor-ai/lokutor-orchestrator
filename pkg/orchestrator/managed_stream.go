...
func NewManagedStream(ctx context.Context, o *Orchestrator, session *ConversationSession) *ManagedStream {
	...
	if o != nil && o.config.FirstSpeaker == FirstSpeakerBot {
		// Signal-driven wait: fire as soon as the transport can deliver
		// audio. The opening is always generated by the configured LLM; no
		// canned speech is embedded in the orchestrator.
		go func() {
			if ms.waitTransportReady(3*time.Second) != nil || ms.ctx.Err() != nil {
				return
			}
			ms.session.AddMessage("user", "The conversation has just started. Give a brief, natural greeting in the configured language and ask how you can help.")
